...
func getTopCrates(input: String) -> String {
    ...
    for instruction in input.instructions {
        let tops = crates[instruction.original - 1].suffix(instruction.count)
        crates[instruction.destination - 1].append(contentsOf: tops)
        crates[instruction.original - 1].removeLast(instruction.count)
    }
